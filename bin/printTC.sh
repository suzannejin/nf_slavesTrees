...
## Prog/REG ##
##############
declare -a flavour="slave_align" #"prog_align"  #"reg_align"  "slave_align"
  ...
  printf "${family};"
    ...
    do
        ...
        do
          ...
          do
            for slaveTree in ${slaveTrees[@]}  ## loop all the buckets
            do
                cat ../results/individual_scores/${family}.${flavour}.${nSeq}.${align_method}.with.${tree_method}.tree.slave.${slaveTree}.tc | tr '' ';'| tr -d "[:space:]"
                #seatoxin.slave_align.1000.CLUSTALO.with.codnd.tree.slave.famsadnd.tc
                printf ";" 
            done
          done
       	done
